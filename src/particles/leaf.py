...
class Leaf:
    def __init__(self, tile_map, assets, image_duration, loop=False):
    ...
    def update(self, dt):
        for rect in self.leaves:
            if random.random() * 49999 <= rect.width * rect.height:
                ...
                self.particles.append(Particle(self.assets, pos, velocity=[-10, 30], frame=random.randint(0, 5), image_duration=self.image_duration, loop=self.loop))
